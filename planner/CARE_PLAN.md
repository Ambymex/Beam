...
**Crons:** `send-due` every minute; `companion-heartbeat` every 30 min but self-throttled to hourly (see `HEARTBEAT_GAP`; to halve wasted invocations the pg_cron itself could go hourly — optional, needs SQL) (SQL in
the setup docs).
